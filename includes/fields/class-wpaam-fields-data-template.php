...
/**
 * wpaam_Fields_Data_Template Class.
 * The profile fields loop class.
 *
 * @since 1.2.0
 */
class wpaam_Fields_Data_Template {
	...
	public function __construct( $args = '' ) {
		...
		$this->groups      = wpaam_get_field_groups( $args );
	...
	public function next_group() {
		...
		$this->group = wpaam_array_to_object( $this->group );

		if( ! empty( $this->group->fields ) ) {
			$this->group->fields = apply_filters( 'wpaam_group_fields', $this->group->fields, $this->group->id );
	...
	public function profile_groups() {
		...
		if( $this->current_group + 1 < $this->group_count ) {
			...
			do_action( 'wpaam_field_groups_loop_end' );
			$this->rewind_groups();
		}
	...
	public function the_profile_group() {
		...
		$wpaam_fields_group = $this->next_group();

		if( 0 === $this->current_group ) {
			do_action( 'wpaam_field_groups_loop_start' );
		}
	...
	public function the_profile_field() {
		...
		$wpaam_field = $this->next_field();

		if ( ! empty( $wpaam_field->value ) ) {
			$value = maybe_unserialize( $wpaam_field->value );
		} else {
			$value = false;
		}
		...
		if ( ! empty( $value ) || ( '0' === $value ) ) {
			...
			if( false === $this->is_visible( $wpaam_field ) )
