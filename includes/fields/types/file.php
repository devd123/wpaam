 ...
 * @since 1.0.0
 */
class wpaam_Field_Type_File extends wpaam_Field_Type {
	...
	public static function options() {
		...
		$options[] = array(
			'name'  => 'extensions',
			'label' => esc_html__( 'Allowed file types', 'wpaam' ),
			'desc'  => esc_html__( 'Enter the extension of the files that can be uploaded through this field, separated with a comma. Example: jpg, png, gif', 'wpaam' ),
			'type'  => 'text',
 		);

		$options[] = array(
			'name'  => 'multiple',
			'label' => esc_html__( 'Allow multiple files', 'wpaam' ),
			'desc'  => esc_html__( 'Enable this option to allow users to upload multiple files through this field.', 'wpaam' ),
			'type'  => 'checkbox',
		);

		$options[] = array(
			'name'  => 'max_file_size',
			'label' => esc_html__( 'Maximum file size', 'wpaam' ),
			'desc'  => esc_html__( 'Enter the maximum file size users can upload through this field. The amount must be in bytes.', 'wpaam' ),
			'type'  => 'text',
		);
