...
/**
 * Abstract wpaam_Form class.
 *
 * @abstract
 * @author      Mike Jolley
 * @author      Alessandro Tesoro
 */
abstract class WPAAM_Form {
	...
	public static function add_error( $error ) {
		//print_r($error);die("Abstract Class");
		self::$errors[] = $error;
	}
	...
	public static function show_errors() {
		foreach ( self::$errors as $error )
			echo '<div class="wpaam-message error"><p class="the-message">' . $error . '</p></div>';
	}
	...
	public static function show_confirmations() {
		foreach ( self::$confirmations as $confirmation )
			echo '<div class="wpaam-message success"><p class="the-message">' . $confirmation . '</p></div>';
	}
	...
	protected static function get_posted_file_field( $key, $field ) {
		$file = wpaam_trigger_upload_file( $key, $field );
	...
	protected static function validate_fields( $values, $form ) {
		...
		foreach ( self::$fields as $group_key => $group_fields ) {
			foreach ( $group_fields as $key => $field ) {
				...
				if ( $field['required'] && empty( $values[ $group_key ][ $key ] ) ) {
					return new WP_Error( 'validation-error', sprintf( __( '%s is a required field', 'wpaam' ), $field['label'] ) );
				}

				// Validate email fields.
				if ( 'email' === $field['type'] && ! is_email( $values[ $group_key ][ $key ] ) ) {
					return new WP_Error( 'email-validation-error', sprintf( __( 'Please enter a valid email address for the "%s" field.', 'wpaam' ), $field['label'] ) );
				}
		...
		return apply_filters( "wpaam/form/validate={$form}", true, self::$fields, $values );
