	...
	class Pretty_Metabox {
		...
		private function check_meta_box( $meta_box ) {
			...
			if( ! array_key_exists( 'id', $meta_box ) ) {
				$errors->add( 'metabox-id-missing', __('Error: metabox must have an id.','wppf', 'wpaam') );
			}

			// Check that the title exists
			if( ! array_key_exists( 'title', $meta_box ) ) {
				$errors->add( 'metabox-title-missing', sprintf( __('Error: metabox "%s" must have a title.','wppf', 'wpaam'), $meta_box['id'] ) );
			}
