	...
	class Pretty_Taxonomy_Metabox {
		...
		private function check_meta_box( $meta_box ) {
			...
			if( ! array_key_exists( 'id', $meta_box ) ) {
				$errors->add( 'metabox-id-missing', sprintf( __('Error: metabox "%s" must have an ID.','wppf', 'wpaam'), self::get_id() ) );
			}

			// Check that the taxonomy exists
			if( ! array_key_exists( 'taxonomy', $meta_box ) ) {
				$errors->add( 'metabox-taxonomy-missing', sprintf( __('Error: metabox "%s" must have a taxonomy parameter assigned.','wppf', 'wpaam'), self::get_id() ) );
			}

			// Check that the taxonomy exists
			if( !is_array($meta_box['taxonomy']) ) {
				$errors->add( 'metabox-taxonomy-array', sprintf( __('Error: metabox "%s" taxonomy parameter must be an array.','wppf', 'wpaam'), self::get_id() ) );
			}
