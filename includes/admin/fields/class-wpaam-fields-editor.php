...
/**
 * wpaam_Fields_Editor Class
 *	
 * @since 1.0.0
 */
class WPAAM_Fields_Editor {
	 ...
	 * @since 1.0.0
	 */
	const editor_hook = 'users_page_wpaam-profile-fields';
	 ...
	 * @since 1.0.0
	 */
	const single_field_hook = 'users_page_wpaam-edit-field';
	...
	public static function editor_page() {
		...
		<div class="wrap wpaam-fields-editor-wrap">

			<h2 class="wpaam-page-title">
				<?php _e( 'WP User Manager - Fields Editor', 'wpaam' ); ?>
				<?php do_action( 'wpaam/fields/editor/title' ); ?>
			</h2>

			<div class="wp-filter">
				<?php echo self::navbar(); ?>
				<?php do_action( 'wpaam/fields/editor/navbar' ); ?>
			</div>
			...
			<div id="nav-menus-frame">
				...
				<div id="menu-management-liquid" class="wpaam-editor-container">

					<?php echo self::group_table(); ?>

					<div class="wpaam-table-loader">
						<span id="wpaam-spinner" class="spinner wpaam-spinner"></span>
					</div>
	...
	private static function navbar() {
		...
		// Get all groups
		$groups = wpaam()->field_groups->get_groups( array( 'order' => 'ASC' ) );

		if( empty( $groups ) ) :

			$output = '<div class="message error"><p>';
				$output .= __('It seems you do not have any field groups. Please deactivate and re-activate the plugin.', 'wpaam');
			$output .= '</p></div>';

		else:

				$output = '<form method="get" action="'. admin_url( 'users.php?page=wpaam-profile-fields' ) .'">';

					$output .= '<input type="hidden" name="page" value="wpaam-profile-fields">';
					...
					$args = array(
						'options'          => $options,
						'label'            => __('Select a field group to edit:', 'wpaam'),
						'id'               => 'wpaam-group-selector',
						'name'             => 'group',
					...
					$output .= '<p>' . wpaam()->html->select( $args );
						$output .= '<span class="submit-btn"><input type="submit" class="button-secondary" value="'.__('Select', 'wpaam').'"></span>';
					$output .= '</p>';
	...
	private static function group_table() {
		...
		$custom_fields_table = new WPAAM_Groups_Fields();
		$custom_fields_table->prepare_items();
		$custom_fields_table->display();

		wp_nonce_field( 'wpaam_fields_editor_nonce', 'wpaam_fields_editor_nonce' );
	...
	public function add_meta_box() {
		add_meta_box( 'wpaam_fields_editor_edit_group', __( 'Group Settings', 'wpaam' ), array( $this, 'group_settings' ), self::editor_hook, 'side' );
		add_meta_box( 'wpaam_fields_editor_help', __( 'Fields Order', 'wpaam' ), array( $this, 'help_text' ), self::editor_hook, 'side' );
	}
	...
	public function single_field_add_meta_box() {
		...
		if( $this->field_object->set_requirement && $this->field->meta !== 'user_email' )
			add_meta_box( 'wpaam_field_requirement', esc_html__( 'Requirement', 'wpaam' ), array( $this, 'requirement_setting' ), self::single_field_hook, 'side' );

		// Add option to display on registration form if it's in primary group.
		if( wpaam()->field_groups->is_primary( intval( $_GET['from_group'] ) ) && $this->field_object->set_registration && $this->field->meta !== 'user_email' )
			add_meta_box( 'wpaam_field_on_registration', esc_html__( 'Show on registration form', 'wpaam' ), array( $this, 'field_on_registration' ), self::single_field_hook, 'side' );

		// Add name adjustment option.
		if( $this->field->meta == 'first_name' || $this->field->meta == 'last_name' )
			add_meta_box( 'wpaam_field_adjust_name', esc_html__( 'Display full name', 'wpaam' ), array( $this, 'name_setting' ), self::single_field_hook, 'side' );

		// Add profile visibility metabox.
		if( $this->field->meta !== 'password' || $this->field->meta !== 'user_avatar' ) {
			add_meta_box( 'wpaam_profile_visibility', esc_html__( 'Visibility', 'wpaam' ), array( $this, 'visibility_settings' ), self::single_field_hook, 'side' );
		}
	...
	public static function help_text( $current_menu = null ) {
		...
		$output = '<p>';
			$output .= sprintf( __('Click and drag the %s button to change the order of the fields.', 'wpaam'), '<span class="dashicons dashicons-menu"></span>');
		$output .= '</p>';
	...
	public static function primary_message() {
		...
		if( isset( $_GET['group'] ) && !wpaam()->field_groups->is_primary( intval( $_GET['group'] ) ) )
			return;
		?>

		<p>
			<span class="dashicons dashicons-info"></span>
			<?php _e('Fields into this group can appear on the signup page.', 'wpaam') ;?>
		</p>
	...
	public function group_settings() {
		...
		$description_args = array(
			...
			'label'        => __('Group description', 'wpaam'),
			'class'        => 'textarea',
		);

		// Prepare delete url
		$delete_url = wp_nonce_url( add_query_arg( array( 'action' => 'delete', 'group' => (int) $this->group->id ), admin_url( 'users.php?page=wpaam-profile-fields' ) ), 'delete', 'nonce' );

		?>

		<form method="post" action="<?php echo admin_url( 'users.php?page=wpaam-profile-fields' ); ?>" id="wpaam-group-settings-edit">

			<div class="wpaam-group-settings">
				<?php echo wpaam()->html->text( $name_args ); ?>
				<?php echo wpaam()->html->textarea( $description_args ); ?>
			</div>

			<div id="major-publishing-actions">
				<div id="delete-action">
					<?php if( !$this->group->is_primary && $this->group->can_delete ) : ?>
						<a class="submitdelete deletion" href="<?php echo $delete_url; ?>"><?php _e('Delete Group', 'wpaam'); ?></a>
					<?php endif; ?>
				</div>
				<div id="publishing-action">
					<input type="hidden" name="wpaam-action" value="edit_group"/>
					<input type="hidden" name="group" value="<?php echo ( isset( $_GET['group'] ) ) ? (int) $_GET['group'] : (int) $this->group->id; ?>"/>
					<?php wp_nonce_field( 'wpaam_group_settings' ); ?>
					<input type="submit" name="publish" id="publish" class="button button-primary button-large" value="<?php _e('Save Group Settings', 'wpaam'); ?>">
				</div>
				<div class="clear"></div>
			</div>
	...
	public function process_group() {
		...
		if( isset( $_GET['action'] ) && $_GET['action'] == 'delete' && isset( $_GET['group'] ) && is_numeric( $_GET['group'] ) ) {
			...
			if( wpaam()->field_groups->delete( (int) $_GET['group'] ) ) {
				...
				$fields = wpaam()->fields->get_by_group( $args );

				foreach ( $fields as $field_to_delete ) {
					wpaam()->fields->delete( $field_to_delete->id );
				}

				// Redirect now.
				$admin_url = add_query_arg( array( 'message' => 'group_delete_success' ), admin_url( 'users.php?page=wpaam-profile-fields' ) );
		...
		if( isset( $_POST['wpaam-action'] ) && $_POST['wpaam-action'] == 'edit_group' ) {

			// nonce verification
			if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'wpaam_group_settings' ) ) {
				return;
			}
			...
			wpaam()->field_groups->update( (int) $_POST['group'], $args );

			// Redirect now
			$admin_url = add_query_arg( array( 'message' => 'group_success' ), admin_url( 'users.php?page=wpaam-profile-fields' ) );
			wp_redirect( $admin_url );
			exit();
	...
	public static function edit_field_page() {
		...
		if( !isset( $_GET['field'] ) || !is_numeric( $_GET['field'] ) )
			...
			$editor_url = add_query_arg( array( 'action' => 'edit', 'group' => $current_group ), admin_url( 'users.php?page=wpaam-profile-fields' ) );
		...
		<div class="wrap wpaam-fields-editor-wrap">
			...
			<h2 class="wpaam-page-title">
				<?php echo __( 'Editing field', 'wpaam' ); ?>
				<a href="<?php echo esc_url( $editor_url ); ?>" class="add-new-h2"><?php _e('Back to editor', 'wpaam'); ?></a>
			</h2>

			<form name="wpaam-edit-field-form" action="#" method="post" id="wpaam-edit-field-form" autocomplete="off">
				<div id="poststuff">
					<div id="post-body" class="metabox-holder columns-2">

						<div id="post-body-content">

							<?php do_action( 'wpaam/fields/editor/single' ); ?>
						...
						<div id="postbox-container-1" class="postbox-container">
							<div id="save-field-container" >
								...
								<div id="save-field" class="postbox">
									<h3 class="hndle ui-sortable-handle"><span><?php _e('Save Field', 'wpaam'); ?></span></h3>
									<div id="major-publishing-actions">
										<div id="publishing-action">

											<input type="hidden" name="wpaam-action" value="save_field"/>
											...
											<?php wp_nonce_field( 'wpaam_save_field' ); ?>

											<input type="submit" name="publish" id="publish" class="button button-primary button-large" value="<?php _e('Save Field', 'wpaam'); ?>"></div>
											<div class="clear"></div>
										</div>
									</div>
								</div>
	...
	public function field_title_editor() {
		...
		$field_name_args = array(
			...
			'label'        => false,
			'placeholder' => __('Enter a name for this field', 'wpaam'),
			'class'        => 'text',
		);
		...
		<div id="titlediv">
			<div id="titlewrap">
				<?php echo wpaam()->html->text( $field_name_args ); ?>
			</div>
		</div>
	...
	public function field_description_editor() {
		...
		<div class="description-editor">
			<h3><?php esc_html_e('Field Description (optional)', 'wpaam'); ?></h3>
	...
	public function requirement_setting() {
		...
		$args = array(
			...
			'label'   => esc_html__('Set this field as required', 'wpaam'),
			'desc'    => esc_html__('Enable to force the user to fill this field.', 'wpaam'),
		);

		echo wpaam()->html->checkbox( $args );
	...
	public function field_on_registration() {
		...
		$args = array(
			...
			'label'   => esc_html__('Display this field on registration', 'wpaam'),
			'desc'    => esc_html__('Enable to display this field on the registration form.', 'wpaam'),
		);
	...
	public function name_setting() {
		...
		$args = array(
			...
			'current' => wpaam_get_field_option( $this->field->id, 'display_full_name' ) ? true : false,
			'label'   => esc_html__( 'Display full name', 'wpaam' ),
			'desc'    => esc_html__( 'Enable to display the user full name instead.', 'wpaam' ),
		);
	...
	public function visibility_settings() {
		...
		$args = array(
			...
			'label'            => esc_html__( 'Field Visibility', 'wpaam' ),
			'desc'             => esc_html__( 'Determine the visibility of this field.', 'wpaam' ),
			'show_option_all'  => false,
			'show_option_none' => false,
			'options'          => wpaam_get_field_visibility_settings()
		);

		echo wpaam()->html->select( $args );
		...
		if( isset( $_POST['wpaam-action'] ) && $_POST['wpaam-action'] == 'save_field' ) {
			...
			if( wpaam()->fields->update( $field_id, $args ) ) {
				...
				$admin_url = add_query_arg( array(
					...
					'action'  => 'edit',
					'group' => $group_id
				), admin_url( 'users.php?page=wpaam-profile-fields' ) );
