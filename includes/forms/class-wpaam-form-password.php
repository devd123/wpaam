 ...
 * @since 1.0.0
 */
class WPAAM_Form_Password extends WPAAM_Form {
	...
	public static function init() {
		...
		if( !isset( $_GET['password-reset'] ) )
			add_filter( 'wpaam/form/validate=password', array( __CLASS__, 'validate_username' ), 10, 3 );

		if( isset( $_GET['password-reset'] ) )
			add_filter( 'wpaam/form/validate=password', array( __CLASS__, 'validate_passwords' ), 10, 3 );

		// Add password meter field
		if( wpaam_get_option('display_password_meter_registration') && isset( $_GET['password-reset'] ) )
			add_action( 'wpaam_after_inside_password_form_template', 'wpaam_psw_indicator' );
	...
	public static function get_password_fields() {
		...
		self::$fields = apply_filters( 'wpaam_password_fields', array(
			'user' => array(
			...
			'password' => array(
				...
				'password_2' => array(
					'label'       => __( 'Re-enter new password', 'wpaam' ),
					'type'        => 'password',
					'required'    => true,
					'placeholder' => '',
					'priority'    => 2
	...
	public static function validate_username( $passed, $fields, $values ) {
		...
		if( is_email( $username ) && !email_exists( $username ) || !is_email( $username ) && !username_exists( $username ) )
			return new WP_Error( 'username-validation-error', __( 'This user could not be found.', 'wpaam' ) );
	...
	public static function validate_passwords( $passed, $fields, $values ) {
		...
		if ( empty( $password_1 ) || empty( $password_2 ) ) {
			return new WP_Error( 'password-validation-error', __( 'Please enter your password.', 'wpaam' ) );
		}

		if ( $password_1 !== $password_2 ) {
			return new WP_Error( 'password-validation-error-2', __( 'Passwords do not match.', 'wpaam' ) );
		}
	...
	public static function process() {
		...
		if ( empty( $_POST['wpaam_submit_form'] ) && empty( $_POST['wpaam_password_form_status'] ) ) {
			return;
		}
		...
		if ( !empty( $_POST['wpaam_password_form_status'] ) && $_POST['wpaam_password_form_status'] == 'recover' ) {

			self::retrieve_password( $values['user'][ 'username_email' ] );

		} else if ( !empty( $_POST['wpaam_password_form_status'] ) && $_POST['wpaam_password_form_status'] == 'reset' ) {

			self::reset_password( $values['password'][ 'password' ], $values['password'][ 'password_2' ], $_POST['wpaam_psw_reset_key'], $_POST['wpaam_psw_reset_login'] );
	...
	public static function reset_password( $password_1, $password_2, $key, $login ) {
		...
		if ( $user instanceof WP_User ) {
			...
			do_action( 'wpaam_user_reset_password', $user );
	...
	public static function check_password_reset_key( $key, $login ) {
		...
		if ( empty( $key ) || ! is_string( $key ) ) {
			self::add_error( __( 'Invalid key.', 'wpaam' ) );
			return false;
		}
	...
	public static function output( $atts = array() ) {
		...
		// Show psw form if not logged in
			...
			get_wpaam_template( 'forms/password-form.php',
				array(
					'atts'            => $atts,
					'form'            => self::$form_name,
