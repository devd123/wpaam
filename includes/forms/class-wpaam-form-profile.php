...
class WPAAM_Form_Profile extends WPAAM_Form {
	...
	public static function init() {
		...
		if( ! is_admin() ) {
			...
			add_filter( 'wpaam_profile_field_value', array( __CLASS__, 'set_fields_values' ), 10, 3 );
	...
	public static function set_fields_values( $default, $field ) {
		...
		switch ( $field['meta'] ) {
			...
			case 'user_avatar':
				...
				break;
			default:
				return apply_filters( 'wpaam_edit_account_field_value', null, $field, self::$user->ID );
				break;
		}
	...
	public static function get_profile_fields() {
		...
		self::$fields = array(
			'profile' => wpaam_get_account_fields()
		);
	...
	public static function validate_nickname( $passed, $fields, $values ) {
		...
		if( get_option('wpaam_permalink') == 'nickname'  ) :
			...
			if( $nickname !== $current_user->user_nicename && wpaam_nickname_exists( $nickname ) )
			...
			switch ( $meta_key ) {
				...
				case 'user_email':
					...
					if( is_email( $meta_value ) ) :
						...
						self::add_error( __( 'Please enter a valid email address.', 'wpaam') );
						return;
					endif;
